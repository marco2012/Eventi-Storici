...
const redirect_uri    = "http://localhost:3000/oauth2callback/" //dove vengo reindirizzato dopo aver accettato permessi dropbox
const client_id       = "v5i2atoizvgsnct" //app id, publicly exposed string that is used by the service API to identify the application.
const client_secret   = "vdnzz190sntd7sk" //app secret is used to authenticate the identity of the application to the service API when the application requests to access a user's account
